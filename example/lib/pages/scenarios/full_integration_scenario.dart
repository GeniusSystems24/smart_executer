...
/// Scenario: Full Integration Dashboard
///
/// This demonstrates a real-world task management dashboard combining:
/// - SmartPagination for task list with infinite scroll
/// - SuperDialog for confirmations and task creation
/// - TooltipCard for task details and quick actions
/// - SmartExecuter for all API operations
...
class _FullIntegrationScenarioState extends State<FullIntegrationScenario> {
  ...
  late SmartPaginationCubit<Task> _tasksCubit;
  ...
  void _initCubit() {
    _tasksCubit = SmartPaginationCubit<Task>(
  ...
  void _updateCounts(List<Task> tasks) {
    setState(() {
      _completedCount = tasks.where((t) => t.completed).length;
      _pendingCount = tasks.where((t) => !t.completed).length;
    });
  }

  /// Toggle task completion with animation dialog
  Future<void> _toggleTask(Task task) async {
    final newStatus = !task.completed;

    await SmartExecuter.run(
      request: () => _dio.patch('/todos/${task.id}', data: {
        'completed': newStatus,
      }),
      context: context,
      ...
      onSuccess: (response) async {
        ...
        if (mounted) {
          _showStatusChangeDialog(task, newStatus);
        }
      },
  ...
  void _showStatusChangeDialog(Task task, bool completed) {
  ...
  Future<void> _deleteTask(Task task) async {
    ...
    await SmartExecuter.run(
      request: () => _dio.delete('/todos/${task.id}'),
      context: context,
      options: ExecuterOptions(
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          ...
          // Task list
          SliverFillRemaining(
            child: SmartPaginationListView<Task>.withCubit(
              cubit: _tasksCubit,
              padding: const EdgeInsets.all(16),
              itemBuilder: (context, items, index) {
                ...
                // Apply filter
                if (_filter == 'completed' && !task.completed) {
                  return const SizedBox.shrink();
                }
                if (_filter == 'pending' && task.completed) {
                  return const SizedBox.shrink();
                }
